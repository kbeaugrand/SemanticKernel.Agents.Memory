...
public static class RankingDemo
{
    ...
    private static void ConfigureChatCompletionService(IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton(serviceProvider =>
        {
            ...
            {
                logger?.LogError(ex, "Failed to configure Azure OpenAI chat completion service due to an Azure Request failure. Please ensure your Azure OpenAI credentials and network connectivity are correct.");
                throw new InvalidOperationException("Failed to configure Azure OpenAI chat completion service due to an Azure Request failure. See inner exception for details.", ex);
            }
            catch (ArgumentException ex)
            {
                logger?.LogError(ex, "Failed to configure Azure OpenAI chat completion service due to invalid arguments (e.g. deployment name, API Key, or endpoint). Please check your Azure OpenAI settings.");
                throw new InvalidOperationException("Failed to configure Azure OpenAI chat completion service due to invalid arguments. See inner exception for details.", ex);
            }
            catch (UriFormatException ex)
            {
                logger?.LogError(ex, "Failed to configure Azure OpenAI chat completion service due to an invalid URI format in the endpoint configuration.");
                throw new InvalidOperationException("Failed to configure Azure OpenAI chat completion service due to an invalid URI format. See inner exception for details.", ex);
            }
        });
    }
